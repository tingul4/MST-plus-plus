...
class SIDLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Add a clamp to ensure non-negative values
        pred = torch.clamp(pred, min=0.0)
        target = torch.clamp(target, min=0.0)

        # Normalize to get probability distributions for each pixel's spectrum
        pred_norm = pred / (torch.sum(pred, dim=1, keepdim=True) + self.eps)
        target_norm = target / (torch.sum(target, dim=1, keepdim=True) + self.eps)
        
        # Compute KL divergence
        kl_div_pred_target = torch.sum(pred_norm * (torch.log(pred_norm + self.eps) - torch.log(target_norm + self.eps)), dim=1)
        kl_div_target_pred = torch.sum(target_norm * (torch.log(target_norm + self.eps) - torch.log(pred_norm + self.eps)), dim=1)
        
        # SID is the sum of the two KL divergences, averaged over all pixels
        sid = kl_div_pred_target + kl_div_target_pred
...
class ERGASLoss(nn.Module):
    ...
    """
    def __init__(self, upscale_factor=1, eps=1e-8):
    ...
    def forward(self, pred, target):
        ...
        # MSE per band
        mse_per_band = torch.mean((pred_reshaped - target_reshaped).pow(2), dim=2) # shape: (batch, channels)
        
        # Mean of target per band
        mean_target_per_band = torch.mean(target_reshaped, dim=2) # shape: (batch, channels)
        
        # Ratio of MSE to squared mean
        ratio = mse_per_band / (mean_target_per_band.pow(2) + self.eps)
        
        # Mean over bands
        mean_ratio = torch.mean(ratio, dim=1) # shape: (batch)
        
        # ERGAS
        ergas = 100 * self.upscale_factor * torch.sqrt(mean_ratio)
...
class DeltaE00Loss(nn.Module):
    ...
    def __init__(self, eps=1e-8):
        ...
        ref_white = torch.tensor([95.047, 100.000, 108.883], dtype=torch.float32).view(1, 3, 1, 1)
    ...
    def _delta_e_00(self, lab1, lab2):
        ...
        delta_h_prime[delta_h_prime.abs() > torch.pi] -= torch.sign(delta_h_prime[delta_h_prime.abs() > torch.pi]) * 2 * torch.pi
        ...
        h_bar_prime[ (h1_prime.abs() > torch.pi) & (h2_prime.abs() > torch.pi) ] += torch.pi

        T = 1 - 0.17 * torch.cos(h_bar_prime - torch.deg2rad(torch.tensor(30.))) + \
                0.24 * torch.cos(2 * h_bar_prime) + \
                0.32 * torch.cos(3 * h_bar_prime + torch.deg2rad(torch.tensor(6.))) - \
                0.20 * torch.cos(4 * h_bar_prime - torch.deg2rad(torch.tensor(63.)))

        S_L = 1 + (0.015 * (L_bar_prime - 50)**2) / torch.sqrt(20 + (L_bar_prime - 50)**2)
